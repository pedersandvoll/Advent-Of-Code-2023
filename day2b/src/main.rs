...
fn main() {
    ...
    if let Some((successful_games, max_cubes)) = puzzle_calculation(puzzle_input) {
        let games_sum: u32 = successful_games.iter().sum();
        let max_sum: u32 = max_cubes.iter().sum();
        println!("Successful Games Sum: {:?}", games_sum);
        println!("Max Cubes Sum: {:?}", max_sum);
    }
}
...
fn puzzle_calculation(games: Vec<Game>) -> Option<(Vec<u32>, Vec<u32>)> {
    let mut successful_games: Vec<u32> = Vec::new();
    let mut unsuccessful_games: Vec<u32> = Vec::new();
    let mut maximum_cubes: Vec<u32> = Vec::new();
    ...
    for game in games {
        ...
        for round in &game.rounds {
            ...
            max_values = (
                max_values.0.max(round.blue),
                max_values.1.max(round.red),
                max_values.2.max(round.green),
            );
        ...
        let product_maximum_values = max_values.0 * max_values.1 * max_values.2;

        if game_successful {
            maximum_cubes.push(product_maximum_values);
            successful_games.push(game.id);
        } else {
            maximum_cubes.push(product_maximum_values);
            unsuccessful_games.push(game.id);
        }
    }

    Some((successful_games, maximum_cubes))
}
